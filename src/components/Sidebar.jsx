...
export default function Sidebar({ projects, activeProjectId, setActiveProjectId, createProject, renameProject, deleteProject, isOpen, toggleSidebar }) {
    ...
    const startEditing = (proj, e) => { e.stopPropagation(); setEditingId(proj.id); setEditName(proj.name); };
    const saveEdit = (e) => { e.stopPropagation(); if (editName.trim()) renameProject(editingId, editName); setEditingId(null); };
        ...
        <>
            ...
            <div className={`sidebar glass-panel ${isOpen ? 'open' : 'closed'}`}>
                ...
                <div className="sidebar-actions">
                    <button className="new-project-btn" onClick={() => createProject('standard')}><Plus size={16} /> Проект</button>
                    <button className="new-agent-btn" onClick={() => createProject('agent')}><Bot size={16} /> Агент</button>
                </div>

                <div className="projects-list">
                    <div className="list-title">ПРОЕКТЫ</div>
                    {projects.map(proj => (
                        <div key={proj.id} className={`project-item ${activeProjectId === proj.id ? 'active' : ''} ${proj.type === 'agent' ? 'agent-item' : ''}`} onClick={() => setActiveProjectId(proj.id)}>
                            {proj.type === 'agent' ? <Bot size={16} className="proj-icon" /> : <Folder size={16} className="proj-icon" />}
                            {editingId === proj.id ? (
                                <div className="edit-mode">
                                    <input autoFocus value={editName} onChange={(e) => setEditName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && saveEdit(e)} onClick={(e) => e.stopPropagation()} />
